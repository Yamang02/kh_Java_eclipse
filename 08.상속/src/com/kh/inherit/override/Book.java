...
//	}
	...
	/*
	 * 1. Object 클래스의 toString() 메소드 오버라이딩 - toString() 메소드는 객체의 문자열 정보를 리턴한다. -
	 * 기본적으로 Object 클래스에서는 "클래스명@16진해시코드"를 리턴하는데 자식 클래스에서 객체에 대한 유용한 문자열 정보를 리턴하도록
	 * 재정의할 수 있다. - print 메소드의 경우 기본적으로 객체의 참조 값을 넘겨주면 내부적으로 참조값.toString() 메소드를
	 * 호출해서 리턴되는 값을 출력한다.
	 * 
	 */
...
//	}
	...
	/*
	 * 2. Object 클래스의 equals() 메소드 오버라이딩 - 매개값으로 전달받은 객체와 동일한 객체라면 true를, 그렇지 않으면
	 * false를 리턴한다. - 원래 Object.equals는 비교하는 Object의 주소값과 매개변수로 입력받는 Object의 주소값을
	 * 비교한다. - String.equals는 이를 오버라이딩하여 데이터값을 비교하여 값을 출력하도록 함. - 자식클래스에서 객체들이 가지고
	 * 있는 필드의 값들을 비교하도록 재정의할 수 있다.
	 *
	 */
