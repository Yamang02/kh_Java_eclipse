...
public class A_Date {
	...
	public void method1() {
		...
		// 기본 생성자로 인스턴스를 생성하면 현재 시스템의 시간에 대한 정보를 제공한다.
		Date today = new Date();
		
		System.out.println(today);
		
		// 1970년 1월 1일 00시를 기준으로 밀리 세컨드 단위까지 표기된다.
		// 한국 표준 시간(KST)으로 출력될 땐 그리니치 평균시(GMT)보다 9시간 증가된(+) 시간으로 표시된다. 
		Date when = new Date(1);
		
		System.out.println(when);
	
		// Deprecated된 생성자를 사용해 인스턴스 생성
		when = new Date(2022 - 1900 , 12 - 1 , 2);		
	
		System.out.println(when);
		System.out.println();
		
		System.out.println(today.getTime()); //밀리세컨드
...
//		SimpleDateFormat sdf = new SimpleDateFormat("yy년-MM월-dd일(E) (a) hh시 mm분 ss초");
		SimpleDateFormat sdf = new SimpleDateFormat("yy년-M월-d일(E) (a) h시 m분 s초"); // 한 자리 시간에서 0을 제외
