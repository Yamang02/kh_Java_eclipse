...
//			list.remove(3.14);
	...
	public void method2() {
		/*
		 * 컬렉션에서 제네릭스(<>)을 사용하는 이유 - 명시된 타입의 인스턴스만 저장하도록 제한을 두기 위해서 사용한다. - 컬렉션에 저장된
		 * 인스턴스를 꺼내서 사용할 때 매번 형 변환을 하지 않아도 된다.
		 * 
		 * 리스트의 정렬 - String, Wrapper 인스턴스들의 경우엔 Comparable 인터페이스가 내부적으로 구현되어 있어서
		 * Collections.sort() 메소드를 통해서 정렬이 가능하다. - 개발자가 작성하는 클래스의 인스턴스도
		 * Collections.sort() 메소드를 통해서 정렬이 가능하게 하고 싶다면 Comparable 인터페이스를 구현하면 된다.
		 * 
		 */
